  ...
  <div class="edit-item">
    <template v-if="prop.edit_type === 'select'">
      <div class="flex aic jcc">
        ...
        <el-select v-model="row[prop.key]" filterable default-first-option :multiple="prop.select_multiple"
          @change="editItem({ key: prop.key, id:row[row_key]})">
    ...
    <template v-if="prop.edit_type === 'input'">
      <el-input v-model="row[prop.key]" @keyup.native.enter="editItem({ key: prop.key, id:row[row_key]})">
      </el-input>
    </template>
    <template v-if="prop.edit_type === 'textarea'">
      <el-input v-model="row[prop.key]" type="textarea" :autosize="{ minRows: 1, maxRows: 2 }"
        @blur="editItem({ key: prop.key, id:row[row_key]})"></el-input>
    </template>
    <template v-if="prop.edit_type === 'number'">
      <el-input v-model.number="row[prop.key]"
        @keyup.native.enter="editItem({ key: prop.key, id:row[row_key]})"></el-input>
    </template>
    <template v-if="prop.edit_type === 'boolean'">
      <el-switch v-model="row[prop.key]" :active-text="prop.active_text || '是'"
    ...
    <template v-if="prop.edit_type ==='relation'">
      <el-select v-model="row[`${prop.key}_id`]" remote filterable :default-first-option="true"
        :remote-method="(query) => getRelation({ row:row, object_class:prop.object_class,query_key:prop.query_key||'name',index: scope.$index, query,key:prop.key })"
        :loading="row.loading"
        @change="(value) => changeRelation({ row:row, index:scope.$index, value, key: prop.key ,object_class:prop.object_class})">
...
module.exports = {
  ...
  methods: {
    ...
    getRelation({ row = {}, query = '', key = '', object_class = '', query_key = 'name' } = {}) {
      if (!row) return
      row.loading = true
