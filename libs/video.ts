...
class CustomVideoHandle {
  ...
  async getVideoData({ video_url = '' as string } = {}) {
  ...
  async convertVideoToImages({ video_url = '', frame_rate = 8, image_type = 'png', blob_url = true } = {}) {
    await this.loadFfmpeg()
    ...
    for (let i = 1; i <= num_frames; i++) {
      ...
      const image_data = await this.ffmpeg.FS('readFile', image_file_name)
  ...
  // 运行复合指令处理视频
  async handleVideoCommand({ video_url = '', blob_url = true, command = [] as string[] } = {}) {
    await this.loadFfmpeg()
    let video_data = await fetchFile(video_url)
    await this.ffmpeg.FS('writeFile', 'input.mp4', video_data)
